...
export const menuActions: MenuAction[] = [
  ...
  },
  {
    ...
    description:
      "Rate of Force Development (RFD) is a measure of explosive strength or how fast the muscle is developing force. RFD is represented by the slope of the load curve. The recording of an RFD test runs for 5 seconds and aims at capturing one well-performed pull about halfway into the recording cycle.\n\nPlease make sure that the device is tared before starting the test. For a valid test result you should not have any slack in the system before performing the pull and you should avoid outputting any force before the actual pull starts.\n\nBy default the 20-80% protocol is used, but you can choose a different mode. In this protocol you can choose to record one side or both. By default the left side is recorded, but you can choose to record the right side instead. You can also choose to pause between sides for a given amount of seconds. This is useful if you want to rest between sides or if you want to record a different side each time.\n\nWe define RFD as the slope of a linear curve passing 1; the intersection of the load curve and 20% of max load and 2; the intersection of the load curve and 80% of max load.\n\nYou can also use a Time-interval protocol to calculate RFD. In that case you need to manually set the onset threshold in order to determine the starting point of the time-window and you need to set your desired time-window. RFD time-interval is calculated as the slope of the linear curve passing 1; the intersection of the load curve and the onset threshold and 2; the intersection of the load curve and the t0 + time-window.",
  },
  {
    id: "repeaters",
    name: "Repeaters",
