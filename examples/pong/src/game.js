import { connect, disconnect, Entralpi, Motherboard, notify, Tindeq, write } from "@hangtime/grip-connect";
let selectedDevice = '';
export function setupDevice(element) {
    element.addEventListener("change", () => {
        ...
        if (selectedDevice === "entralpi") {
            return connect(Entralpi, async () => {
                ...
                //   if (data && data.value) {
                //     if (data.value.massTotal !== undefined) {
                //       console.log(data.value.massTotal)
                //     } else {
                //       console.log(data.value)
                //     }
                //   }
...
const Ball = {
    new(incrementedSpeed, canvas) {
        return {
            width: 18,
            height: 18,
            x: canvas ? canvas.width / 2 - 9 : 0,
            y: canvas ? canvas.height / 2 - 9 : 0,
...
const Paddle = {
    new(side, canvas) {
        return {
            width: 18,
            height: 70,
            x: side === "left" || !canvas ? 150 : canvas.width - 150,
            y: canvas ? canvas.height / 2 - 35 : 0,
            score: 0,
            move: Direction.IDLE,
            speed: 10,
            wins: 0,
...
const Game = {
    ...
    initialize() {
        ...
        this.player = Paddle.new.call(this, "left", this.canvas);
        this.paddle = Paddle.new.call(this, "right", this.canvas);
        this.ball = Ball.new.call(this, undefined, this.canvas);
        this.paddle.speed = 8;
    ...
    draw: function () {
        ...
        this.context.fillText(String(rounds[Pong.round] ? rounds[Pong.round] : rounds[Pong.round - 1]), this.canvas.width / 2, 100);
    },
    loop: function () {
        Pong.update();
        Pong.draw();
