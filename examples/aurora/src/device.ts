...
function getBoardDeviceOption(key: AuroraBoardName): BoardDeviceOption {
  ...
  if (!fallbackOption) {
    throw new Error("No Aurora board options configured")
  }

  return BOARD_DEVICE_OPTIONS.find((option) => option.key === key) ?? fallbackOption
}

function setSelectedDeviceBoard(boardName: AuroraBoardName): void {
  const nextDeviceOption = getBoardDeviceOption(boardName)
  if (nextDeviceOption.key === currentDeviceOption.key) {
    return
  }
  ...
  currentDeviceOption = nextDeviceOption
  device = currentDeviceOption.create()
  updateConnectButton()
}
...
function updateConnectButton(): void {
  ...
  deviceConnectButton.textContent = `Connect ${currentDeviceOption.label}`
  ...
  element.addEventListener("click", async () => {
    ...
    try {
      await device.connect(async () => {
        await device.led(getPayloadPlacements())
      })
    } finally {
...
function renderConfigurator() {
        ...
        <label>
          ...
          <select id="board-select">
            ${boardOptions
              .map(
                (board) =>
                  `<option value="${board.name}" ${board.name === currentConfig.boardName ? "selected" : ""}>${board.label}</option>`,
              )
              .join("")}
          </select>
        </label>
  ...
  const boardSelect = configuratorElement.querySelector<HTMLSelectElement>("#board-select")
  ...
  boardSelect?.addEventListener("change", () => {
    if (!isAuroraBoardName(boardSelect.value)) {
      return
    }
...
function getPayloadPlacements(): AuroraLedPlacement[] {
  const payloadPlacements: AuroraLedPlacement[] = []
...
function updateURL() {
  ...
  currentUrl.searchParams.set("board", currentConfig.boardName)
